...
var requestData = {
	aggs: {
		...
		state_prov_1: {
			terms: { field: "state_prov", size: 50 }
		},
		family_2: {
			terms: { field: "family", size: 50 }
		},
		genus_3: {
...
function applyFacetToggle(facetContainerId, maxVisible = 5) {
	const container = document.getElementById(facetContainerId);
	const items = Array.from(container.querySelectorAll('.facet-link'));

	if (items.length <= maxVisible) return; // No need for toggle

	// Initially hide everything after maxVisible
	...
	toggleButton.className = 'btn btn-link btn-xs facet-toggle';
	toggleButton.textContent = 'More';

	toggleButton.addEventListener('click', () => {
		const expanded = toggleButton.textContent === 'Less';
...
function renderResults(results) {
	...
	results.forEach(function (doc) {
		...
		var row = `<tr data-source='${JSON.stringify(source)}'>
			<td>${source.relatedinformation|| "N/A"}</td>
	...
	function renderTable(id, headers, rows, title) {
		...
		rows.forEach((row) => {
			...
			row.forEach((cell) => {
				...
				td.textContent = (cell === null || cell === undefined || cell === "") ? "N/A" : cell;
				tr.appendChild(td);
			});
...
function buildRequestData() {
	...
	// Get scientific name filter from input field
	const name = $('#scientificNameSearch').val().trim();
	if (name) {
		must.push({ match: { scientific_name: name } });
	}

	// Add all selected facet filters
	Object.entries(selectedFacets).forEach(([field, values]) => {
		values.forEach(value => {
...
function performSearchWithScientificName(name) {
	// Set the input field (optional but ensures sync)
	$('#scientificNameSearch').val(name);

	// Just fetch results — buildRequestData() will include the name
	fetchResults();
}
